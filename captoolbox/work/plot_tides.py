...
scale = np.arange(0, 1.8, .05)  # scale factor
...
for k, (xi,yi) in enumerate(locations):
    ...
    print 'calculating location:', k+1
    ...
    for ss in scale:
        ...
        COR = ishelf + \
                '\nError after applying tide correction w/different scale factors'
    ...
    plt.axvline(x=opt_scale, color='r')
    plt.ylabel('Std (m)')
