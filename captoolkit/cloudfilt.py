"""
Filter out cloud-contaminated points in ICESat data.
...
def rename_file(fname, suffix='_CFILT'):
    path, ext = os.path.splitext(fname)
    os.rename(fname, path + suffix + ext)


# Define command-line arguments
parser = argparse.ArgumentParser(description='Filter cloud points')
parser.add_argument(
        'files', metavar='file', type=str, nargs='+',
        help='file(s) to process (HDF5)')
parser.add_argument(
        '-v', metavar=('cloud_flg'), dest='vname', type=str, nargs=1,
        help=('Variable for cloud flag '),
        default=['cloud_flg'],)
parser.add_argument(
