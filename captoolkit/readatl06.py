...
def tiffread(ifile):
	...
	"""
	geotiff_da = rioxarray.open_rasterio(ifile)
	geotiff_ds = geotiff_da.to_dataset('band')

	x = np.asarray(geotiff_ds['x'])
	y = np.asarray(geotiff_ds['y'])
	z = np.asarray(geotiff_ds[1])
	...
	proj = geotiff_ds['spatial_ref'].attrs['crs_wkt']
...
if fmask is not None:
    ...
    if fmask.endswith('.tif'):
        ...
        (Xm, Ym, Zm, dX, dY, Proj) = tiffread(fmask)
    ...
    for k in range(len(group)):
        try:
            ...
            with h5py.File(ifile, 'r') as fi:
                ...
                n_fit_photon = fi[group[k]+"/land_ice_segments/fit_statistics/n_fit_photons"][:]
                sigma_h_mean = fi[group[k]+"/land_ice_segments/fit_statistics/sigma_h_mean"][:]
        ...
        # Raster mask
            ...
            i_m = interp2d(Xm, Ym, Zm, x, y, order=0)
        ...
        lat, lon, h_li, s_li, t_dt, h_rb, s_fg, snr, q_flag, f_sn, tide_earth, tide_load, tide_ocean, tide_pole, dac,\
            rgt, cycle, beam, spot, n_fit_photon,sigma_h_mean = lat[flag], lon[flag], h_li[flag], s_li[flag], t_dt[flag], h_rb[flag], s_fg[flag], \
            snr[flag], q_flag[flag], f_sn[flag], tide_earth[flag], tide_load[flag], \
            tide_ocean[flag], tide_pole[flag], dac[flag], rgt[flag], cycle[flag],\
            beam[flag], spot[flag], n_fit_photon[flag], sigma_h_mean[flag]
        ...
        if len(lat[i_asc]) > 1:
            ...
            vardictasc = {'orb': orb[i_asc][::ksam], 'lon': lon[i_asc][::ksam], 'lat': lat[i_asc][::ksam],
                          'h_elv': h_li[i_asc][::ksam], 't_year': t_li[i_asc][::ksam],
                          'n_fit_photon': n_fit_photon[i_asc][::ksam],
                          'sigma_h_mean': sigma_h_mean[i_asc][::ksam],
                          'spot_number': spot[i_asc][::ksam],
                          't_sec': t_gps[i_asc][::ksam]}

            with h5py.File(ofile.replace('.h5', ostr), 'w') as fa:

                # Create output file and loop trough variables
                [fa.create_dataset(k, data=d[:]) for k, d in list(vardictasc.items())]
        ...
        if len(lat[i_des]) > 1:
            ...
            vardictdes = {'orb': orb[i_des][::ksam], 'lon': lon[i_des][::ksam], 'lat': lat[i_des][::ksam],
                          'h_elv': h_li[i_des][::ksam], 't_year': t_li[i_des][::ksam],
                          'n_fit_photon': n_fit_photon[i_des][::ksam],
                          'sigma_h_mean': sigma_h_mean[i_des][::ksam],
                          'spot_number': spot[i_des][::ksam],
                          't_sec': t_gps[i_des][::ksam]}

            with h5py.File(ofile.replace('.h5', ostr), 'w') as fd:

                # Create output file and loop trough varibales
                [fd.create_dataset(k, data=d[:]) for k, d in list(vardictdes.items())]
