...
def _sigma_filter(x, y, n_sigma=3, frac=1/3.):
    ...
    range_ = x[idx].max() - x[idx].min()
    # Detrend
    trend = sm.nonparametric.lowess(y[idx], x[idx], frac=frac, it=2, delta=0.01*range_)[:,1] # CHANGED
    y2[idx] = y[idx] - trend
...
def detrend(x, y, lowess=False, frac=1/3.):
    ...
    if lowess:
        range_ = x.max() - x.min()
        y_trend = sm.nonparametric.lowess(y, x, frac=frac, it=2, delta=0.01*range_)[:,1] # CHANGED
...
def linefit(x, y, return_coef=False):
    ...
    y_fit = sm.RLM(y, X, M=sm.robust.norms.HuberT(), missing="drop").fit(maxiter=2,tol=0.001) # CHANGED
...
def get_scatt_cor(t, h, bs, lew, tes, proc='dif'):
    ...
    try:
        ...
        model = sm.OLS(h_, A_, missing="drop").fit(method='qr') # CHANGED
