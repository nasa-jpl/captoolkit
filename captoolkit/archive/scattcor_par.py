...
def main(ifile, vnames, wnames, dxy, proj, radius=0, n_reloc=0, proc=None):
    ...
    #TODO: Parallelization should start here

    # Loop through nodes
    for k in xrange(N_nodes):

        #TODO: par_func(datain, gridin, gridout, i)

        def par_func(data, grid_xy, grid_out, k):

        if (k % 1000) == 0:
            print 'Calculating correction for node', k, 'of', N_nodes, '...'

        #xi, yi = x_nodes[k], y_nodes[k]
        xi, yi = grid_xy[k,0], grid_xy[k,1]
