...
Notes:
    ...
    The software currently works best using polar-stereo coordinates
    (3031 oe 3413) so please use these for now!
    ...
    At each crossover location the surface slope is also computed using the
    difference between height and distance from the two closest points used
    for the crossover interpolation for each track. The slopes are saved in
    units of degrees.

    If you are using external tile-mode "-f" please provide input path as a
    strings: './tiles/*_a.h5' './tiles/*_d.h5'
...
def get_args():
    ...
    parser.add_argument(
            ...
            help='maximum interpolation distance from crossing (m)',
            default=[350],)
    parser.add_argument(
    ...
    parser.add_argument(
            ...
            help='along-track sample every n:th pnt for each file',
            default=[1],)
    parser.add_argument(
...
if proj != "4326":
    ...
    print("Warning you are using EPSG 4326 which can have issues! \
            Please Polar Stereographic Projections.")
...
def main(ifile1, ifile2):
    ...
    if dxy != 0:
        ...
        # Get bounding box
        bboxs1 = np.ones(len(xp1))
        bboxs2 = np.ones(len(xp2))
    ...
    for k in ibox:
        ...
        for orb_id1 in orb_ids1:
            ...
            for orb_id2 in orb_ids2:
                ...
                tai = interp1d(xa[0:nobs],ta[0:nobs], kind=mode,\
                        fill_value='extrapolate')(xi)
                tbi = interp1d(xb[0:nobs],tb[0:nobs], kind=mode,\
                        fill_value='extrapolate')(xi)

                # Interpolate height to crossover location
                hai = interp1d(ta[0:nobs], ha[0:nobs],kind=mode,\
                        fill_value='extrapolate')(tai)
                hbi = interp1d(tb[0:nobs], hb[0:nobs],kind=mode,\
                        fill_value='extrapolate')(tbi)

                # Distance between to closes points
                d_a = np.sqrt((xa[0]-xa[1])**2 + (ya[0]-ya[1])**2)
                d_b = np.sqrt((xa[0]-xa[1])**2 + (ya[0]-ya[1])**2)

                # Height difference between closest points
                h_a = np.abs(ha[0] - ha[1])
                h_b = np.abs(hb[0] - hb[1])

                # Slope in degrees from closest points
                s_a = np.arctan(h_a / d_a) * 180 / np.pi
                s_b = np.arctan(h_b / d_b) * 180 / np.pi
                ...
                if flag_bs:
                    ...
                    if diff is True:
                        ...
                        bai = interp1d(ta[0:nobs], ba[0:nobs],kind=mode,\
                                        fill_value='extrapolate')(tai)
                        bbi = interp1d(tb[0:nobs], bb[0:nobs],kind=mode,\
                                        fill_value='extrapolate')(tbi)
                ...
                if flag_le:
                    ...
                    if diff is True:
                        ...
                        lai = interp1d(ta[0:nobs], la[0:nobs],kind=mode,\
                                    fill_value='extrapolate')(tai)
                        lbi = interp1d(tb[0:nobs], lb[0:nobs],kind=mode,\
                                    fill_value='extrapolate')(tbi)
                ...
                if flag_ts:
                    ...
                    if diff is True:
                        ...
                        sai = interp1d(ta[0:nobs], sa[0:nobs],kind=mode,\
                                fill_value='extrapolate')(tai)
                        sbi = interp1d(tb[0:nobs], sb[0:nobs],kind=mode,\
                                fill_value='extrapolate')(tbi)
